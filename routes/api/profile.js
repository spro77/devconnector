...
router.get('/user/:user_id', async (req, res) => {
  try {
    ...
    // We can catch some error type and return an corresponding message instead 'Server Error'
  ...
  try {
    const options = {
      uri: `https://api.github.com/users/${
        req.params.username
      }/repos?per_page=5&sort=created:acc&client_id=${config.get('githubClientId')}&client_secret=${config.get(
        'githubSecret'
      )}`,
      method: 'GET',
